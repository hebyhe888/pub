...
namespace NetCorePro.Midleware
{
    ...
    public class JwtTokenIssue
    {
        ...
        public static string IssueJWT(TokenModel tokenModel, TimeSpan expiresSliding, TimeSpan expiresAbsoulte)
        {
            ...
            issuer: "netcore_bussinessAPI",//jwt签发者,非必须
            ...
            signingCredentials: new Microsoft.IdentityModel.Tokens
                .SigningCredentials(new SymmetricSecurityKey(Encoding.ASCII.GetBytes("netcore_bussinessAPI bussiness's Secret Key")),
                SecurityAlgorithms.HmacSha256));//使用私钥进行签名加密
            var encodedJwt = new JwtSecurityTokenHandler().WriteToken(jwt);//生成最后的JWT字符串
            BussinessMemoryCache.AddMemoryCache(encodedJwt, tokenModel, expiresSliding);//将JWT字符串和tokenModel作为key和value存入缓存
            return encodedJwt;
        }
    }
}
